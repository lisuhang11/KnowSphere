...
def _key_from_record(rec: dict[str, Any] | None) -> str | None:
    ...
    provider_id = runtime_provider(rec.get("source"), params)
    spec = get_provider(provider_id)
    key = params.get("api_key")
    if isinstance(key, str) and key.strip() and key.strip() not in PLACEHOLDER_KEYS:
        return key.strip()
    if provider_id == "ollama" or (spec and not spec.requires_auth):
        return "ollama" if provider_id == "ollama" else "EMPTY"
    if provider_id == "siliconflow" and _env_api_key_ok():
